...
import com.intree.development.databinding.AspectExistingImagesGalleryFragmentBinding
...
import com.intree.development.domain.transferrable_objects.AspectPhotoItemTO
import com.intree.development.presentation.home.aspect.vm.AspectViewModel
import dagger.hilt.android.AndroidEntryPoint

@AndroidEntryPoint
class AspectExistingImagesGalleryFragment : Fragment() {

    private val args: AspectExistingImagesGalleryFragmentArgs by navArgs()

    private var _binding: AspectExistingImagesGalleryFragmentBinding? = null

    private val viewModel by viewModels<AspectViewModel>()

    private var photoDataList: MutableList<AspectPhotoItemTO> = ArrayList()

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        return inflater.inflate(R.layout.aspect_existing_images_gallery_fragment, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)

        _binding = AspectExistingImagesGalleryFragmentBinding.bind(view)
                ...
                override fun onCreateViewHolder(
                    ...
                    return object : RecyclerView.ViewHolder(
                        LayoutInflater.from(parent.context).inflate(
                            R.layout.list_item_aspect_gallery_draggable,
                            parent,
                            false
                        )
                    ) {
        ...
        viewModel.aspectFirestoreMetadata.observe(viewLifecycleOwner, {
            if (it!!.baseParams.gallery.isNotEmpty()) {
                photoDataList = it.baseParams.gallery.sortedBy { photoItem -> photoItem.idx }
                    .map { item -> AspectPhotoItemTO(isSelected = false, idx = item.idx, item) }.toMutableList()
        ...
        photoDataList.forEach {
            ...
            if (counter == photoDataList.size) {
                Handler(Looper.myLooper()!!).postDelayed({
                    val action = AspectExistingImagesGalleryFragmentDirections.actionRoomExistingImagesGalleryFragment2ToCreateRoomFragment2(args.roomId)
